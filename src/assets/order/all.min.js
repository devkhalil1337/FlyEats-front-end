...
var PaymentOptions = { cash: false, card: true };
...
var StickySidebar = function () {
  ...
  var StickySidebar = function () {
    ...
    function StickySidebar(sidebar) {
      ...
      this.container = this.sidebar ? this.sidebar.parentElement : null;
...
MyApp.angular.controller('IndexPageController', ['$scope', '$http', 'InitService','myMethods','$location','$rootScope','$filter','$timeout','$anchorScroll','$window','ServerServices', function ($scope, $http, InitService,myMethods,$location,$rootScope,$filter,$timeout,$anchorScroll,$window,ServerServices) {
  ...
  InitService.addEventListener('ready', function () {
    ...
    $scope.Pages = {home:'',orderNow:'',contactUs:'',currentPage:'',delivery:'',collection:'',table:''};
    ...
    $scope.chargeExtraPostcodesList = [
      'KT20', 'KT3', 'SW17', 'SW16', 'KT5', 'SE25'
    ];

    $scope.labelExtraCharges = false;
  ...
  if(checkNetConnection()) {
    myMethods.getAddressAPI($scope.number, $scope.CompId).then(function(response) {
      if(response.status == 200) {
        ...
        separateCollectionAddress($scope.myAddresses, $scope);
...
$scope.selectAddress = function(address) {
    ...
    const indexExtra = $scope.getIndexOfSelectedPostCode($scope.chargeExtraPostcodesList, address.custPostalCode);
    if (indexExtra !== -1) {
      $scope.labelExtraCharges = true;
      $scope.DeliveryCharges = 3.50;
      addDeliveryAmount();
    } else {
      $scope.labelExtraCharges = false;
      $scope.DeliveryCharges = 0;
...
//Custom Filters
 ...
 $scope.OrderType = function(type) {
     var VatType = $scope.restaurantDetails.lenght ? $scope.restaurantDetails[0].S_VARType : '';
...
$scope.setPaymentOption = () => {
  if (PaymentOptions.card && PaymentOptions.cash) {
    ...
    $scope.paymentmethod = PaymentOptions.card ? 'Card' : 'Cash';
  }
}
...
$scope.AddSelectionProducts = function(selection, modifier, bool) {
  if(selection.Type == "M") {
    ...
    if ($scope.Selections[$scope.IndexCount].MinimumSelection === 0) {
      $scope.isButtonDisable = true;
    }
  }
};
          ...
          if($scope.OrderTypeVar == "Collection" || $scope.OrderTypeVar == "TableOrder") {
            ...
            if(deliveryAddressId == 0) {
              ...
              setTimeout(function() {
                $scope.ChangePage('checkout-details');
                  setTimeout(function() {
                    if (!PaymentOptions.cash && PaymentOptions.card) {
...
$scope.openCart = function() {
  ...
  if (checkNetConnection()) {
    setTimeout(function(){
      if($scope.RestaurtantTime['restOn'] && $scope.RestaurtantTime.openTime !== $scope.RestaurtantTime.closeTime){
        ...
        if($scope.OrderTypeVar != "") {
          ...
          swal(
            ...
            $scope.RestaurtantTime.openTime !== $scope.RestaurtantTime.closeTime ? "Opening time " + $scope.RestaurtantTime.openTime  + " to " + $scope.RestaurtantTime.closeTime : "",
            "error"
          );
        }
    },500);
...
$scope.isLoyaltyProgramEnabled = async () => {
  const loyalty = await myMethods.checkLoyaltyStatus($scope.CompId)
    .catch(err => console.log('loyaltyErr=>>>',err));
...
$scope.getIndexOfSelectedPostCode = (arrayPostcodes, selectedPostalCode) => {
  return arrayPostcodes.findIndex(x => {
...
$scope.checkPostCode = async () => {
  ...
  if (indexAddress !== -1) {
    const selectedPostalCode = $scope.myAddresses[indexAddress].custPostalCode;
    ...
    const indexPostcode = $scope.getIndexOfSelectedPostCode(arrayPostcodes, selectedPostalCode);
    if (indexPostcode !== -1) {
      return true;
    } else {
      const indexExtra = $scope.getIndexOfSelectedPostCode($scope.chargeExtraPostcodesList, selectedPostalCode);
      if (indexExtra !== -1) {
        $scope.DeliveryCharges = 3.50;
        addDeliveryAmount();
        return true;
...
$scope.completeOrder = async () => {
  $scope.deleteEmptyAddres();
  if (!deliveryAddressId && $scope.OrderTypeVar === 'Delivery') {    
...
$scope.evaluatePrices = () => {
  if (($scope.subTotalAmount + $scope.DeliveryCharges) === $scope.TotalAmount && $scope.VoucherDiscount) {
    $scope.TotalAmount -= $scope.VoucherDiscount;
  }
}
...
$scope.addCardDetails = async function(cardNumber, month, year, ccv, postalcode) {
  $scope.evaluatePrices();
  showLoader();
  $scope.paymentError = '';
...
//      getProducts();
  ...
  // setTimeout(function(){
    if($scope.RestaurtantTime['restOn'] && $scope.RestaurtantTime.openTime !== $scope.RestaurtantTime.closeTime){
      ...
      myMethods.makeChargeIntents(Company_ID, totalAmount).then(result => {
        $scope.stripeVar.confirmCardPayment(
          result.data[0].Msg,
          {
            payment_method: {
              ...
              billing_details: {
                name: $scope.number + ' / ' + $scope.userEmail,
              },
              metadata: {
...
$scope.modifierQuantity = (arr, item, index) => {
  ...
  for (let arrIndex = 0; arrIndex < item.quantity; arrIndex++) {
...
$scope.sendOrder = function() {
 ...
 //     $scope.DeliveryTime
  ...
  //       Recvorder_Totaldiscountpercentage: voucherTotalDiscount,
      ...
      $.ajax({
       method: 'POST',
       url: API_Services + "Service.asmx/AddReceiveOrder2",
       //type: "GET",
...
$scope.sendEmail = function(contactUs) {
  ...
  if(isEmptyError) {
    ...
    myMethods.sendTableBookingEmail(data).then(res => {
        ...
        contactUs.guest ? 'Booking Done.' : 'Message Sent.'
        , "success");
        $scope.contactUs = {
          name: '',
          email: '',
...
//    $locationProvider.hashPrefix('');
    ...
    .when("/main", {
      ...
      templateUrl: "js/views/my-rewards.html",
      controller: "IndexPageController"
    }).when("/myaddress", {
...
MyApp.angular.factory('myMethods', function($http, $timeout, $location) {
    return {
      ...
      makeChargeIntents: async (companyId, totalAmount) => {
        return await $http({
          method: "GET",
          url: API_Services + "service.asmx/MakeCharge_intents",
          dataType: 'json',
          params: {
            totalAmount: totalAmount,
            CompID: companyId
...
function findOrderType(orderNatureType) {
  ...
  function separateCollectionAddress(arr, $scope) {
    var addresstype = "";
    var counter = 0;
    for(var i = 0; i < arr.length; i++) {
      ...
      if(addresstype == "default") {
        ...
        const indexExtra = $scope.getIndexOfSelectedPostCode($scope.chargeExtraPostcodesList, arr[i].custPostalCode);
        if (indexExtra !== -1) {
          $scope.labelExtraCharges = true;
        }
      } else {
